...
# # 打印最相似的 top_n 个结果
# print("vector_db(documents):", results['documents'][0])
# # 打印最相似的 top_n 个分数(距离)
# print("vector_db(distances):", results['distances'][0])
# print('-' * 100, '*' * 100, '-' * 100)
'''
...
def hybrid_search(query, top_k=3, bm25_weight=0.5):
    ...
    #     对 combined_scores 数组中的值进行降序排序，并返回排序后的索引值,argsort索引下标的升序，-1就成了降序
    ...
    hybrid_results = []
    for i in top_index[:top_k]:
        hybrid_results.append(outputs[i])
